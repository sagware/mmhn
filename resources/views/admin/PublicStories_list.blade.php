...
<head>
	<meta charset="utf-8" />
	<title>Innovation Stories List| Materials and Manufacturing in Healthcare Network Public Stories List <?php echo date('d/M/Y h:m');?></title>
                        ...
                        <div class="panel-body">
                            <table id="data-table" class="table table-striped table-bordered display" cellspacing="0" width="100%">
								...
								@foreach($gra as $g)
                                    <tr >
										<td>{{$ct}}</td>
										 <td><a href="/public_review/{{$g->id}}" title="Review and approve">Review/Approve</a></td>
										<td>{{$g->status}}</td>
                                        <td>{{$g->title}}</td>
										<td>{{$g->category}}</td>

		...
		if(!empty($request->file('pic'))){
			...
			$pathToFile ="";
			 ...
			 $data = array('name'=>$name,'pathTo'=>$pathToFile, 'email'=>$receiver, 'm'=>$mes, 'user'=>'sagir','user_name'=>$sender);
			  ...
			  Mail::send('emails.admin_challenge_not', $data, function ($m) use ($sag) {
					$m->from('admin@materialsinhealth.org', 'no-reply: Challenge Edit Submission for your Approval');
					$m->to($sag['mail'], $sag['mail'])->subject('no-reply: Challenge  Edit Submission for your Approval');
				 });
		}
...
return redirect()->intended('/challengesubmitted');
	...
	//show about us
			...
			$c = PublicStories::where("id",$id)->first();
				...
				return back()->with("like","");
					...
					if(in_array($uid,unserialize($c->likes))){
						return back()->with("liked","");
					}else{
					array_push(unserialize($c->likes),$uid);
					$c->likes = serialize($c->likes);
					$c->save();
					return back()->with("like","");
			...
			return view('admin.needdetailapprove')->with("p",$p)->with("r",$r);
			...
			if(empty($u)){	 
			 ...
			 $data = array('name'=>$name,'pathTo'=>$pathToFile, 'email'=>$receiver, 'm'=>$mes, 'user'=>'sagir','user_name'=>$sender);
			  ...
			  Mail::send('emails.approveneed', $data, function ($m) use ($sag) {
					$m->from('admin@materialsinhealth.org', 'Challenge Approval');
			...
			return back()->with("approve",'approved');
			...
			$u = User::where("id",$p->posted_by)->first();		 
			 ...
			 $data = array('name'=>$name,'pathTo'=>$pathToFile, 'email'=>$receiver, 'm'=>$mes, 'user'=>'sagir','user_name'=>$sender);
			  ...
			  Mail::send('emails.rejectneed', $data, function ($m) use ($sag) {
					$m->from('admin@materialsinhealth.org', 'Challenge Rejection');
			...
			$u = User::where("id",$p->posted_by)->first();		 
			 ...
			 $data = array('name'=>$name,'pathTo'=>$pathToFile, 'email'=>$receiver, 'm'=>$mes, 'user'=>'sagir','user_name'=>$sender);
			  ...
			  Mail::send('emails.revisionneed', $data, function ($m) use ($sag) {
					...
					$m->to($sag['mail'], $sag['mail'])->subject('no-reply: Challenge Revision Request');
				 });
			}
			return back()->with("revise",'revise');
			...
			//echo $id; exit;
			$p = PublicStories::where("id", ">","0")->where("category","need")->where("posted_by",$id)->where("status","approved")->orderBy("updated_at","DESC")->simplePaginate(4);
			$r = PublicStories::where("id",">",0)->where("category","need")->where("status","approved")->orderBy("updated_at")->take(5)->get();
	...
	public function news(){
		...
		$p = PublicStories::where("posted_by",Auth::user()->id)->where('status', "approved")->where(function($query){
